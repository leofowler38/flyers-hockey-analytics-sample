...
team_wins %>%
  ...
  select(Team, Home_Wins, Away_Wins, Total_Wins,
...
ggplot(team_wins, aes(x = reorder(Team, Home_Advantage), y = Home_Advantage)) +
  ...
  coord_flip() +
  labs(
  title = "Home Ice Advantage by Team (Home - Away Win%)",  # was −
  x = "Team",
  y = "Difference in Win % (Home - Away)"                   # was −
  ) +
...
```{r}
correlation <- cor(nhl_away_dist$Travel_Miles, nhl_away_dist$Away_Win_Pct, use = "complete.obs")

ggplot(nhl_away_dist, aes(x = Travel_Miles, y = Away_Win_Pct)) +
  geom_point(size = 3, color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "red", size = 1) +
  labs(
    title = "NHL Team Away Win Percentage vs Travel Distance (2024-25 NHL Season)",
...
```{r}
season_2024 <- season_2024 %>%
  mutate(Attendance_num = readr::parse_number(as.character(Attendance)))

attendance_summary <- season_2024 %>%
  group_by(Home) %>%
  summarise(Avg_Attendance = mean(Attendance_num, na.rm = TRUE), .groups = "drop") %>%
  rename(Team = Home)

nhl_attendance <- team_wins %>%
  left_join(attendance_summary, by = "Team")

cor_adv_att <- cor(nhl_attendance$Avg_Attendance,
...
ggplot(nhl_attendance, aes(x = Avg_Attendance, y = Home_Advantage)) +
  ...
  labs(
    ...
    y = "Home Advantage (Home Win% − Away Win%)"
  ) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  theme_minimal(base_size = 13) +
  theme(
